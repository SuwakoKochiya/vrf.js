...
function prove(pk, sk, m) {
    ...
    const x1 = expandSecret(utils_1.B(sk));
    ...
    const k = utils_1.OS2IP(utils_1.bufferReverse(expandSecret(utils_1.B(ks))));
    const c = hashPoints(g, h, P1, r, P2, h.mul(k));
    const s = k.sub(c.mul(x).mod(q));
    return Buffer.concat([ECP2OS(r), utils_1.B(utils_1.I2OSP(c, N)), utils_1.B(utils_1.I2OSP(s, N2))]);
}
exports.prove = prove;
function proofToHash(pi) {
    return utils_1.B(pi.slice(1, N2 + 1));
}
